...
public interface IOrderRepository extends JpaRepository<OrderEntity, Long>, JpaSpecificationExecutor<OrderEntity> {
    ...
    @Query("SELECT new webtoon.main.payment.dtos.OrderPendingDTO(o.id,o.user_id,o.maDonHang, o.subs_pack_id) FROM tbl_order o join tbl_payment p on o.id = p.orderId.id where o.user_id.id = ?1 and o.status = 'PENDING_PAYMENT' and o.paymentMethod='VN_PAY'")
    ...
    @Query("select count(o) from tbl_order o where o.status != 'DRAFTED' and function('date_format', o.createdAt, '%Y, %m, %d') >= function('date_format', CURRENT_DATE, '%Y, %m, %d')")
    Long countTotalOrderInToday();


    @Query("select sum(o.finalPrice) from tbl_order o where o.status = 'COMPLETED' and function('date_format', o.createdAt, '%Y, %m, %d') >= function('date_format', CURRENT_DATE, '%Y, %m, %d')")
    Long sumTotalRevenueInToday();

    @Query("select count(o) from tbl_order o where o.status = 'USER_CONFIRMED_BANKING' and function('date_format', o.createdAt, '%Y, %m, %d') >= function('date_format', CURRENT_DATE, '%Y, %m, %d')")
    Long countTotalPaymentPendingInToday();
    @Query("select count(o) from tbl_order o where o.status = 'COMPLETED' and function('date_format', o.createdAt, '%Y, %m, %d') >= function('date_format', CURRENT_DATE, '%Y, %m, %d')")
    Long countTotalCompletedOrderInToday();

    @Query("select count(o) from tbl_order o where o.status = 'CANCELED' and function('date_format', o.createdAt, '%Y, %m, %d') >= function('date_format', CURRENT_DATE, '%Y, %m, %d')")
...
//            "\n", nativeQuery = true)
    ...
    // for mariadb
    @Query(value = "SELECT DATE_FORMAT(o.created_at, '%m') as m, COUNT(o.id) as v from`tbl_order` as o \n" +
            "where o.order_type = 'RENEW' AND :year = DATE_FORMAT(o.created_at, '%Y') AND o.status = 'COMPLETED'\n" +
...
//            "GROUP BY m.d\n" +
//            "\n", nativeQuery = true)
    @Query(value = "SELECT DATE_FORMAT(u.can_read_until_date, '%m') as m, COUNT(u.id) as v from `tbl_user` as u \n" +
            "where DATE_FORMAT(u.can_read_until_date, '%Y') = :year AND u.can_read_until_date is not null AND u.can_read_until_date <= CURRENT_DATE\n" +
            "GROUP BY m", nativeQuery = true)
