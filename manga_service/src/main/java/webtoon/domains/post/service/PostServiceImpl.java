...
public class PostServiceImpl implements IPostService{
    ...
    public PostServiceImpl(IPostRepository postRepository, ITagRepository tagRepository, ITagRelationRepository tagRelationRepository, PostMapper postMapper) {
    ...
    public PostDto savePost(PostInput input) {
        ...
        if (input.getTagRelations() != null) {
            ...
            input.getTagRelations().forEach(t -> {
                TagEntity tag = this.tagRepository
                        .findByTagName(t)
                        .orElse(TagEntity.builder().tagName(t).build());
                TagEntityRelation relation = this.tagRelationRepository.findByObjectIDAndTagType(entity.getId(), "post");
                if (relation != null) {
                    tags.add(relation);
                }
                else
                    tags.add(TagEntityRelation.builder()
